  ...
  if [ "$1" -a "$2" ]; then
    ...
    while read line; do
      if echo "$line" | grep -q "crypted"; then
        ...
        cat "${cryptpassword}" | cryptsetup --type plain --cipher=serpent-xts-plain64 --key-size=512 --hash=sha512 --key-file=- --batch-mode open "$dev" "plain-${dev_partuuid}"
        touch "$FOLD/crypttab"
...
install_kernel() {
  if [ "$1" ]; then
    execute_chroot_command "DEBIAN_FRONTEND=noninteractive apt update && apt install -y console-data console-common tzdata locales keyboard-configuration openssh-client openssh-server && apt install -y linux-image-generic grub-pc"; EXITCODE=$?
    return $EXITCODE
  fi
}
